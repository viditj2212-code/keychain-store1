      ...
      <div className="w-full xl:w-[60%] xl:sticky xl:top-24 space-y-8 animate-fade-in">
        <div className="relative group rounded-[3rem] overflow-hidden bg-gray-900 border border-gray-100 shadow-2xl transition-all duration-700">
          ...
          <div className="absolute bottom-10 left-10 z-10">
            <div className="glass p-8 rounded-[2.5rem] flex items-center gap-8">
              <div className="space-y-1">
                <p className="font-outfit text-[9px] font-bold text-gray-400 uppercase tracking-[0.3em] italic">Current Spec</p>
                <h3 className="font-space text-2xl font-extrabold text-gray-900 uppercase italic tracking-tighter">{product.name}</h3>
              </div>
              <div className="w-px h-12 bg-gray-900/10"></div>
              <div className="space-y-1">
                <p className="font-outfit text-[9px] font-bold text-gray-400 uppercase tracking-[0.3em] italic">Validation</p>
                <span className="font-outfit text-sm font-extrabold text-gray-900 flex items-center gap-3 uppercase italic tracking-widest">
          ...
          <div className="absolute top-10 left-10 z-10 flex flex-col gap-4">
            {product.isNew && <Badge variant="primary" className="shadow-2xl px-6 py-2.5 font-space italic">N° SERIES-X</Badge>}
            {product.isFeatured && <Badge variant="secondary" className="shadow-2xl px-6 py-2.5 font-space italic">CORE-EDITION</Badge>}
          </div>
        </div>
      ...
      <div className="w-full xl:w-[40%] space-y-16 py-10">
        <div className="space-y-8">
          <div className="flex items-center gap-6">
            <span className="font-outfit px-5 py-2 bg-gray-900 text-white text-[9px] font-bold uppercase tracking-[0.4em] rounded-lg italic">
              Serial ID: {product.id.split('-')[0]}
            </span>
            <div className="h-0.5 flex-1 bg-gray-900/5"></div>
          </div>

          <h1 className="font-space text-5xl md:text-[6.5rem] font-extrabold text-gray-900 leading-[0.85] tracking-tighter uppercase italic">
            {product.name}
          </h1>

          <p className="font-outfit text-lg md:text-xl text-gray-400 font-bold uppercase tracking-[0.2em] leading-tight max-w-xl italic">
            {product.description}
          </p>
        </div>
        ...
        <div className="space-y-10 pt-12 border-t border-gray-100">
          <div className="flex items-baseline gap-8">
            {product.salePrice ? (
              <>
                <span className="font-space text-7xl font-extrabold text-gray-900 tracking-tighter uppercase italic">${product.salePrice.toFixed(2)}</span>
                <span className="font-space text-3xl text-gray-200 line-through font-bold tracking-tighter uppercase italic">${product.price.toFixed(2)}</span>
              </>
            ) : (
              <span className="font-space text-7xl font-extrabold text-gray-900 tracking-tighter uppercase italic">${product.price.toFixed(2)}</span>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="p-1 items-center bg-gray-50/50 rounded-[2rem] border border-gray-100 flex gap-6 pr-8 transition-all hover:bg-white hover:shadow-xl duration-500">
              <div className="w-16 h-16 bg-gray-900 rounded-2xl shadow-xl flex items-center justify-center text-white font-space font-bold italic">
                STR
              </div>
              <div className="space-y-0.5">
                <p className="font-outfit text-[9px] font-bold text-gray-400 uppercase tracking-[0.3em] italic">Strength</p>
                <p className="font-outfit text-sm font-bold text-gray-900 uppercase tracking-widest italic">Industrial</p>
              </div>
            </div>
            <div className="p-1 items-center bg-gray-50/50 rounded-[2rem] border border-gray-100 flex gap-6 pr-8 transition-all hover:bg-white hover:shadow-xl duration-500">
              <div className="w-16 h-16 bg-gray-900 rounded-2xl shadow-xl flex items-center justify-center text-white font-space font-bold italic">
              ...
              <div className="space-y-0.5">
                <p className="font-outfit text-[9px] font-bold text-gray-400 uppercase tracking-[0.3em] italic">Material</p>
                <p className="font-outfit text-sm font-bold text-gray-900 uppercase tracking-widest italic">Aerospace</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-10 pt-12 border-t border-gray-100 mb-10">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-10 bg-gray-900 p-2 rounded-[1.8rem]">
              ...
              <span className="font-space text-2xl font-extrabold text-white w-8 text-center">{quantity}</span>
              <button
            ...
            <div className="grid grid-cols-1 gap-6">
              {product.features.map((feature, index) => (
                <div key={index} className="group flex items-center justify-between p-8 rounded-[1.8rem] bg-white border border-gray-100 hover:border-gray-900 transition-all duration-500">
                  <span className="font-outfit text-gray-900 font-bold text-sm tracking-widest uppercase italic">{feature}</span>
